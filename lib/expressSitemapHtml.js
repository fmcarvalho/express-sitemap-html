...
function buildPrev(prev, regexp, path) {
    ...
    regexp = regexp.replace(/(\\\/)/g, '/') // replaces all occurrences of "\\/" with "/"
    regexp = regexp.replace(/(\/)$/g, '') // replaces last occurrence of "/" with "" IF the URL ends with a "/", as something else puts a "/" at the end of the url.
